...
FLAT_LIST_INSTRUCTION = (
    "Format numbered lists as a single flat list — never split it into "
    "multiple separate numbered lists, and never nest bullet points "
    "under a numbered item. For example, instead of:\n"
    "1. Skill A\n- detail\n2. Skill B\n- detail\n"
    "write:\n"
    "1. Skill A: detail\n2. Skill B: detail"
)
...
def get_skills_advice(jobs_text: str) -> str:
    return call_agent(
        ...
        "Keep it under 300 words. " + FLAT_LIST_INSTRUCTION,
        jobs_text,
    )
...
def get_interview_prep(jobs_text: str) -> str:
    return call_agent(
        ...
        "proactively during the interview. Keep it under 300 words. " + FLAT_LIST_INSTRUCTION,
        jobs_text,
    )
